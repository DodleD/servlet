...
<body>
	...
	<div class="outer">
		...
		<form action="<%=contextPath%>/update.me" id="enroll-form"
			...
			<table>
				<tr>
					...
					<td><input type="text" name="userId" maxlength="12"
						value="<%=userId%>" required readonly></td>
					<td></td>
				</tr>
				<tr>
					<td>* 이름</td>
				...
				<tr>
					...
					<td colspan="2"><input type="checkbox" name="interest"
						...
						for="movie">영화</label> <input type="checkbox" name="interest"
						id="etc" value="기타"> <label for="etc">기타</label></td>
				</tr>
			</table>
			<script>
                ...
                //가져온 checkbox요소들을 전부 반복한다.
                for(let input of inputArr){ //in으로하면 index가 들어오고 of로하면 요소가 들어옴, interest에 input.value값이 포함되어 있다면 true
			...
			<div align="center">
				<button type="submit" class="btn btn-sm btn btn-secondary" onclick="return checkPwd();">정보변경</button>
				<button type="button" class="btn btn-primary btn btn-sm btn" data-toggle="modal"
					data-target="#update-pwd-modal">비밀번호변경</button>
				<button type="button" class="btn btn-sm btn btn-danger" data-toggle="modal" data-target="#delete-modal">회원탈퇴</button>
			</div>
		</form>
	</div>
				...
				<div class="modal-body" align="center">
                    ...
                    <script>
                        document.getElementById("edit-pwd-btn").onclick = function(){
                            ...
                            // if($("input[name=updatePwd]").val() !== $("input[name=checkPwd]").value()){
                            // }
			...
			<div class="modal-content">
				...
				<div class="modal-body" align="center">
                   <form action="<%=contextPath%>/delete.me">
					...
					<br><br>
					<input type="hidden" name="userId" value="<%=userId%>">
					비밀번호 : <input type="password" name="userPwd" required method="POST">
					<br><br>
