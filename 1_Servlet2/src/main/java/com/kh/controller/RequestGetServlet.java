...
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//체크박스와 같이 복수의 value값들을 추출하고자할 때는 
		...
		*/
		...
		//즉, 여기 Java코드 내에 사용자가 보게될 응답 html 구문을 작성할 것이다.
		
		//장점 : java코드 내에 작성하기 때문에 반복문이나 조건문, 유용한 메소드 같은 것들을 활용하여 동적인 응답을 내릴 수 있음.
		//단점 : 불편, 복잡, 혹시라도 차후에 html을 수정한다면 java코드를 수정하는 것이기 때문에 수정된 내용을
		//다시 반영시키고자 서버를 재실행 해야된다.
		
		// response객체를 통해서 사용자에게 html(응답화면) 전달
		// 1) 이제부터 내가 출력할 내용은 문서형태 html이고 문자셋은 utf-8이다 -> 선언
		response.setContentType("text/html; charset=utf-8");
		
		// 2) 응답하고자하는 사용자(요청했던 사람)와의 스트림(클라이언트와의 연결통로)를 생성
